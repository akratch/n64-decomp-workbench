...
verdict-to-lever mapping in the code cites numbers and carries its own one-line
action for each, so a lever whose section is not in the shipped revision still
prints something a reader can act on. `decomp-workbench guide 20` on a revision
that stops at lever 19 answers:
