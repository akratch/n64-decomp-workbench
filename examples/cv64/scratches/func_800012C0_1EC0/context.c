...
#define DIFFICULTY_AND_SPAWN_SETTINGS(difficulty,spawn_settings) (((difficulty << 4) & 0xF0) | (spawn_settings & 0x0F))
...
#define CHECK_EVENT_FLAGS(event_flag_ID,value) BITS_HAS(sys.SaveStruct_gameplay.event_flags[event_flag_ID], value)
#define SET_EVENT_FLAGS(event_flag_ID,value) BITS_SET(sys.SaveStruct_gameplay.event_flags[event_flag_ID], value)
#define REMOVE_EVENT_FLAGS(event_flag_ID,value) BITS_UNSET(sys.SaveStruct_gameplay.event_flags[event_flag_ID], value)
...
#define CUTSCENE_OVERLAY_FILM_REEL 1
...
#define INTERACTABLES_VARIABLE_1(id) (id + 1)
...
#define CONT_BTNS_HELD(controller_id,buttons) BITS_HAS(GET_CONTROLLER(controller_id).btns_held, (buttons))
#define CONT_BTNS_PRESSED(controller_id,buttons) BITS_HAS(GET_CONTROLLER(controller_id).btns_pressed, (buttons))
#define CONT_ALL_BTNS_HELD(buttons) BITS_HAS( ((GET_CONTROLLER(0).btns_held) | (GET_CONTROLLER(1).btns_held) | (GET_CONTROLLER(2).btns_held) | (GET_CONTROLLER(3).btns_held)), (buttons) )
#define CONT_ALL_BTNS_PRESSED(buttons) BITS_HAS( ((GET_CONTROLLER(0).btns_pressed) | (GET_CONTROLLER(1).btns_pressed) | (GET_CONTROLLER(2).btns_pressed) | (GET_CONTROLLER(3).btns_pressed)), (buttons) )
...
extern OSContStatus controller_status[4];

enum FadeFlag {
    FADE_WITH_OUTLINE = (1 << (0)),
    FADE_IN = (1 << (14)),
    FADE_OUT = (1 << (15))
};
...
typedef struct Actor {
    ...
    u8 padding1[4];
    Model* model;
    u8 field_0x28[sizeof(Object) - 0x28];
    ActorConfig* settings;
} Actor;
...
extern item_model_settings item_model_settings_list[41];
...
enum SaveFlag {
    SAVE_FLAG_GAME_WAS_SAVED_MID_PLAY = (1 << (0)),
    SAVE_FLAG_EASY = (1 << (4)),
    SAVE_FLAG_NORMAL = (1 << (5)),
    SAVE_FLAG_HARD = (1 << (6)),
    SAVE_FLAG_HARD_MODE_UNLOCKED = (1 << (8)),
    SAVE_FLAG_HAVE_REINHARDT_ALT_COSTUME = (1 << (9)),
    SAVE_FLAG_HAVE_CARRIE_ALT_COSTUME = (1 << (10)),
    SAVE_FLAG_REINDHART_GOOD_ENDING = (1 << (17)),
    SAVE_FLAG_CARRIE_GOOD_ENDING = (1 << (18)),
    SAVE_FLAG_REINDHART_BAD_ENDING = (1 << (19)),
    SAVE_FLAG_CARRIE_BAD_ENDING = (1 << (20)),
    SAVE_FLAG_VAMP_CURED_USING_ITEM = (1 << (29)),
    SAVE_FLAG_COSTUME_IS_BEING_USED = (1 << (30)),
    SAVE_FLAG_CAN_EXPLODE_ON_JUMPING = (1 << (31))
};
...
typedef enum CutsceneFlag {
    CUTSCENE_FLAG_PLAYING = (1 << (0)),
    CUTSCENE_FLAG_FILM_REEL_EFFECT = (1 << (2)),
    CUTSCENE_FLAG_DISPLAY_WIDESCREEN_BORDERS = (1 << (3)),
    CUTSCENE_FLAG_IS_ENTRANCE_CUTSCENE = (1 << (4)),
    CUTSCENE_FLAG_PLAY_DURING_CUTSCENE_STATE_AFTER_IT_ENDS = (1 << (5)),
    CUTSCENE_FLAG_80000000 = (1 << (31))
} CutsceneFlag;
...
extern cutscene_parameters cutscene_settings[61];
...
typedef enum cv64_player_flag {
    PLAYER_FLAG_RUNNING = (1 << (0)),
    PLAYER_FLAG_JOGGING = (1 << (1)),
    PLAYER_FLAG_WALKING = (1 << (2)),
    PLAYER_FLAG_CROUCHING = (1 << (3)),
    PLAYER_FLAG_PLAY_TIRED_SFX_WHILE_IDLING_WITH_LOW_HEALTH = (1 << (5)),
    PLAYER_FLAG_OPENING_DOOR = (1 << (6)),
    PLAYER_FLAG_ACTION_PERFORMED = (1 << (7)),
    PLAYER_FLAG_ATTACKING_FROM_ABOVE = (1 << (8)),
    PLAYER_FLAG_ATTACKING_IN_LEVEL = (1 << (9)),
    PLAYER_FLAG_ATTACKING_FROM_BELOW = (1 << (10)),
    PLAYER_FLAG_MELEE_ATTACK = (1 << (11)),
    PLAYER_FLAG_B_ATTACK = (1 << (12)),
    PLAYER_FLAG_SUBWEAPON_ATTACK = (1 << (13)),
    PLAYER_FLAG_AIRBORNE = (1 << (14)),
    PLAYER_FLAG_JUMP_UP = (1 << (15)),
    PLAYER_FLAG_JUMPING = (1 << (16)),
    PLAYER_FLAG_RECOVER_FROM_FALL_DAMAGE = (1 << (17)),
    PLAYER_FLAG_DAMAGED = (1 << (18)) | (1 << (19)),
    PLAYER_FLAG_STANDING_UP_AFTER_BEING_ATTACKED_DOWN = (1 << (17)) | (1 << (18)) | (1 << (19)),
    PLAYER_FLAG_SLOW_DOWN = (1 << (20)),
    PLAYER_FLAG_SLIDING = (1 << (21)),
    PLAYER_FLAG_STO = (1 << (26)),
    PLAYER_FLAG_VAMP = (1 << (27)),
    PLAYER_FLAG_POISON = (1 << (28)),
    PLAYER_FLAG_TIRED = (1 << (29)),
    PLAYER_FLAG_DAMAGED_AT_LEAST_ONCE = (1 << (30)),
    PLAYER_FLAG_DEAD = (1 << (31))
} cv64_player_flag_t;
...
typedef struct {
    ...
    Vec3 field_0x02;
    Vec3f field_0x08;
    f32 height_targetGfx_player;
    f32 reticle_display_distance;
    animation_info spinning_anim_info;
    Actor* targeted_enemy_ptr;
    s16 field_0x34;
    s16 field_0x36;
    Actor* red_targeted_enemy_ptr;
    Vec3f enemy_position;
    Vec3f distance_enemy_player;
    Actor* facing_enemy_ptr;
    Vec3f field_0x58;
    Actor* field_0x64;
...
typedef struct {
    ...
    u8 field_0x30[sizeof(Object) - 0x30];
} Player;
...
typedef struct {
    ...
    u8 field_0x28[12];
    cameraMgr_data* data;
    u8 field_0x38[sizeof(Object) - 0x38];
} cameraMgr;
...
typedef enum EffectVisualFlag {
    EFF_VISUAL_FLAG_DESTROY = (1 << (3)),
    EFF_VISUAL_FLAG_EMIT_LIGHT = (1 << (4)),
    EFF_VISUAL_FLAG_ANIMATE_SCALE = (1 << (5)),
    EFF_VISUAL_FLAG_NO_VARIABLE_TRANSPARENCY = (1 << (7)),
    EFF_VISUAL_FLAG_200 = (1 << (9)),
    EFF_VISUAL_FLAG_DONT_FADE_OUT_WHEN_MARKED_FOR_DELETION = (1 << (14)),
    EFF_VISUAL_FLAG_DONT_ANIMATE =
        (1 << (18)),
    EFF_VISUAL_FLAG_HIDE = (1 << (19)),
    EFF_VISUAL_FLAG_200000 = (1 << (21)),
    EFF_VISUAL_FLAG_LOOK_AT_CAMERA_YAW = (1 << (22)),
    EFF_VISUAL_FLAG_LOOK_AT_CAMERA_PITCH = (1 << (23)),
    EFF_VISUAL_FLAG_CREATE_MODEL_AND_SET_CHILD_AFTER_PARENT = (1 << (26)),
    EFF_VISUAL_FLAG_CREATE_MODEL_AND_SET_CHILD = (1 << (27)),
    EFF_VISUAL_FLAG_20000000 = (1 << (29))
} EffectVisualFlag;
typedef enum EffectAnimInfoVisualFlag {
    EFF_ANIM_VISUAL_FLAG_VARIABLE_TRANSPARENCY = (1 << (1))
} EffectAnimInfoVisualFlag;
typedef enum EffectAnimInfoFlag {
    EFF_ANIM_FLAG_ACTIVE = (1 << (13))
} EffectAnimInfoFlag;
...
typedef struct EffectAnimationInfo {
    ...
    u8 field_0x07;
    s16 field_0x08;
    u16 flags_and_number_of_frames;
    u8 field_0x0C[4];
} EffectAnimationInfo;
...
typedef enum InteractableID {
    ...
    INTERACT_ID_NO_VANISH_POT_POURRI = 0x0A,
    ...
    INTERACT_ID_NO_VANISH_WOODEN_STAKE = 0x10,
    INTERACT_ID_NO_VANISH_ROSE = 0x11,
    INTERACT_ID_NO_VANISH_THE_CONTRACT = 0x12,
    INTERACT_ID_NO_VANISH_ENGAGEMENT_RING = 0x13,
    ...
    INTERACT_ID_NO_VANISH_INCANDESCENT_GAZE = 0x18,
...
enum InteractableConfigFlag {
    ITEM_VANISH_OR_UPDATE_POSITION = (1 << (0)),
    TEXT_SPOT_DESTROY_AFTER_INTERACTION = (1 << (1)),
    TEXT_SPOT_DESTROY_IF_EVENT_FLAG_IS_SET = (1 << (2)),
    TEXT_SPOT_DISABLE_IF_EVENT_FLAG_IS_NOT_SET = (1 << (3)),
    TEXT_SPOT_DO_ACTION_AFTER_SELECTING_OPTION = (1 << (4)),
    TEXT_SPOT_IF_YES_START_CUTSCENE = (1 << (5)),
    TEXT_SPOT_IF_YES_CHANGE_ACTOR_STATE = (1 << (6)),
    TEXT_SPOT_IF_YES_SET_EVENT_FLAG = (1 << (7)),
    TEXT_SPOT_IF_YES_PULL_LEVER = (1 << (8)),
    ITEM_DOES_NOT_FLASH = (1 << (10)),
    ITEM_INVISIBLE = (1 << (11))
};
...
void heap_init(
    ...
    heaps[kind].size = ((((u32) heap_size) + 7) & ~7);
    heaps[kind].heap_start = ((((u32) first_block_ptr) + 7) & ~7);
...
void heap_writebackDCache(void) {
    Heap* first;
    Heap* current_heap;
    first = &heaps[0], current_heap = &heaps[(s32)(sizeof(heaps) / sizeof(heaps[0])) - 1];
    do {
...
void initHeaps(void) {
    s32 i;
    for (i = (s32)(sizeof(heaps) / sizeof(heaps[0])) - 1; i > 0; i--) heaps[i].flags = HEAP_INACTIVE;
    heap_init(
        HEAP_KIND_MULTIPURPOSE,
...
GraphicContainerHeader* GraphicContainer_Alloc(HeapKind heap_kind, u32 size) {
    ...
    ((data_header->flags) |= (HEAP_BLOCK_GRAPHIC_CONTAINER));
...
void GraphicContainer_Free(void* ptr) {
    u8* temp;
    temp = temp + ((u8*) ptr - temp) - ((u32) & ((HeapBlockHeader*) 0)->graphic_container);
